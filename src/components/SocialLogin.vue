    ...
    loginWithGoogle () {
      this.$gAuth
        ...
        .then(GoogleUser => {
          ...
          console.log('GoogleUser', GoogleUser.getBasicProfile().kR)
          console.log('GoogleUser', GoogleUser.getBasicProfile().nt)
          var userInfo = {
            loginType: 'google',
            google: GoogleUser,
            idt: GoogleUser.getBasicProfile().kR,
            emailId: GoogleUser.getBasicProfile().nt

          }
          axios.post('http://127.0.0.1:5008/api/login', { email: userInfo.emailId, idt: userInfo.idt })
